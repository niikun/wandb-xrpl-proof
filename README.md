...
## Weave Demo — OpenAI × Weave × IncrementalAnchor

`demo/weave_demo.py` shows the simplest path from a `@weave.op()` function to XRPL checkpoints in three additions:
...
run = wandb.init(project="traces-quickstart")   # 1. start a W&B run

@weave.op()
def extract_dinos(sentence: str) -> dict:
    ...  # any OpenAI / LLM call

with IncrementalAnchor(run, chunk_size=2) as anchor:  # 2. wrap with anchor
    for i in range(5):
        result, call = extract_dinos.call(sentence)   # 3. use .call() to get Weave trace
        anchor.log({"step": i}, weave_call=call)      #    log metrics + Weave hashes together

# print XRPL explorer links for each checkpoint
...
# → prints XRPL testnet URLs for each checkpoint
# → saves weave_proof.json (used for verification)
```

**Verify — walk prev links, recompute hash chain, confirm no tampering:**
...
|---|---|
| 1. Collect | Walk XRPL `prev` links from final tx back to genesis |
| 2. Structure | `seq` continuity (0, 1, 2…) and `prev` link matches actual tx hash |
| 3. Hash chain | `SHA-256((chain_{i-1} or "") + chunk_hash_i)` matches on-chain `commit_hash` |
...
demo/
├── weave_demo.py     # Weave × IncrementalAnchor minimal demo (OpenAI + anchor + verify)
├── run_demo.py       # IncrementalAnchor × Weave × XRPL demo (--samples N / --chunk-size K)
├── verify_demo.py    # proof verification (--chain --proof / --tx TX_HASH)
├── weave_proof.json  # proof file generated by weave_demo.py
